...
local this = {
    ...
    mod = require("src.HitboxViewer.data.mod"),
...
local function write_strings_to_config(strings, table, color, ignore_values)
    for _, name in ipairs(strings) do
        if ignore_values and util_table.table_contains(ignore_values, name) then
            goto continue
        end
        table.disable[name] = false
        if color then
...
function this.init()
    ...
    write_fields_to_config(
        ...
        config.default.hitboxes.damage_type,
        config.default_color
    )
    write_fields_to_config(
        "app.HitDef.DAMAGE_ANGLE",
        config.default.hitboxes.damage_angle,
        config.default_color
    )
    write_fields_to_config(
        "app.Hit.GUARD_TYPE",
        config.default.hitboxes.guard_type,
        config.default_color
    )
    write_fields_to_config(
        "app.PressDef.PRESS_LEVEL",
        config.default.pressboxes.press_level,
        config.default_color
    )
    write_fields_to_config(
    ...
    write_strings_to_config(
        ...
        config.default.hitboxes.misc_type,
        config.default_color
    )
    write_strings_to_config(
